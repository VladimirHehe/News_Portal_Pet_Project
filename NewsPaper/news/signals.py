...
from .tasks import send_newsletter_task, welcome_email_task
...
def send_newsletter(sender, instance, action, **kwargs):
    if action == 'post_add':
        ...
        for category in categories:
            ...
            for subscription in subscriptions:
                user = subscription.user
                send_newsletter_task.apply_async(args=[instance.pk, user.pk, category.pk])
...
def welcome_email(sender, instance, created, **kwargs):
    if created:
        welcome_email_task.apply_async(args=[instance.pk])
